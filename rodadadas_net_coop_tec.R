...
inv_2019 <- raw_list_2019[[grep("_coop", names(raw_list_2019))]] %>% 
...
d <- nt1 %>% 
  dplyr::select( R_Start_ID,  R_Destiny_ID, R3c, R3a, R3b, R_Start_ID_ego) %>%
...
nt_all <- bind_rows(d, nt2) %>% 
  dplyr::mutate(R3b = if_else(is.na(R3b), "9", R3b))%>% 
  dplyr::left_join(., groups[[name_rodada]] %>% 
                     dplyr::select(ID_node, V1), by = c("R_Start_ID" = "ID_node") ) 
...
net_felipe_ego_grC <- net_graphs(edge_list = nt_all, 
                                 ...
                                 out_file = paste0(out_dirs[[name_rodada]], "/cooptec_grC_net.png"))
...
writexl::write_xlsx(to_save, path = paste0(out_dirs[[name_rodada]],"/cooptec_grC_net_results.xlsx"))
...
nt1 <- nt1 %>% 
  dplyr::mutate(id = paste0(R_Start_ID, "_", R_Start_ID_ego)) %>% 
  dplyr::filter(!id  %in% c("42_a","42_c","49_a","49_b","76_a")) %>% 
  dplyr::select(-id, -contains("cometario")) 
...
d2 <- nt1 %>%
  ...
  #dplyr::filter(R3a != 2) %>%
  #dplyr::filter(R3b != 9) %>% 
  dplyr::select(R_Start_ID, R_Destiny_ID, starts_with("R3"))
#dplyr::left_join(.,  get_questions_opts(data = preguntas$rodada_2019, var = "R3a", var_lab = "label"),  by = c("R3a", "Opcoes_numero"))

net_felipe_ego <-net_graphs(edge_list = d2, 
                            groups = groups[[name_rodada]], 
                            gr_colors = gr_colors[[name_rodada]], 
                            preguntas = preguntas[[name_rodada]], 
                            out_file = paste0(out_dirs[[name_rodada]], "/cooptec_standard_net_graph.png"))
...
writexl::write_xlsx(to_save, path = paste0(out_dirs[[name_rodada]],"/cooptec_standard_net_results.xlsx"))
...
d2 <- d2 %>% 
  bind_rows(., dopler) %>%
  dplyr::select(R_Start_ID = start, R_Destiny_ID = end, R3c, R3a, R3b, R3b) %>% 
...
net_felipe_ego_dir <- net_graphs_dir(edge_list = d2, 
                                     ...
                                     gr_colors = gr_colors[[name_rodada]], 
                                     preguntas = preguntas[[name_rodada]], 
                                     out_file = paste0(out_dirs[[name_rodada]], "/cooptec_Dir_net.png"))
...
writexl::write_xlsx(to_save, path = paste0(out_dirs[[name_rodada]],"/cooptec_Dir_net_results.xlsx"))
...
inv_2020 <- raw_list_2020[[grep("_coop", names(raw_list_2020))]] %>% 
  dplyr::select(everything(.), R3b = R3d) 
...
saveRDS(net,  paste0(out_dirs[["rodada_2019"]],"/cooptec_edge_list_JF.rds"))
...
d2 <- nt1 %>%
  ...
  # dplyr::filter(R1a != 2) %>%
  # dplyr::filter(R1b != 9) %>% 
  dplyr::select(R_Start_ID, R_Destiny_ID, starts_with("R3"), "R_b3")
