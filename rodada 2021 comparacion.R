  ...
  if(rodada == "2020"){
    to_likert <- data %>% 
      ...
      dplyr::mutate(selected_var = as.character(selected_var),
                    selected_var = replace_na(selected_var, 0)) 
...
template_pth <- "D:/OneDrive - CGIAR/Documents/PPA 2021/Database SNA PPA 2020 A_B (03ago21)_para_CIAT_(correção).xlsx"
file_pth <- "D:/OneDrive - CGIAR/Documents/PPA 2021/Database SNA PPA 2020 A_B (03ago21)_para_CIAT_(correção).xlsx"
  #"D:/OneDrive - CGIAR/Documents/Database SNA PPA 2019 final 2020 06 08.xlsx"
out_dir <- paste0("D:/OneDrive - CGIAR/Documents/PPA 2021/questionaire_results/rodada ", rodada, "/")

## hoja del archivo de excel
sheets <- readxl::excel_sheets(template_pth)

## opciones de respuesta

opts <- readxl::read_xlsx(template_pth, sheet = "Todos_detalhado")
  ...
  dplyr::mutate(vars_opts = purrr::map2(.x = root_var, .y = Metrica, .f = function(.x, .y){
    ...
    if(grepl(pattern ='múltipla escolha', .y)){    
      ...
      ret <- opts %>% 
        ...
        dplyr::filter(complete.cases(Opcoes_numero))
...
miembros_sheet <- readxl::read_xlsx(template_pth, sheet = "Orgs_grupo_rodadas")
...
if(rodada == "2020"){
  ...
  df_trial <- percepcoes_df_cleaned %>% 
    ...
    dplyr::left_join(., groups %>% dplyr::select(ID_node,Nome, V1), by = c("ID" = "ID_node")) 
...
df_sec_eco <- raw_list %>% 
  purrr::pluck(., grep("Empresas_orgs_atributos", names(raw_list))) %>% 
  dplyr::select(ID, V40a) %>% 
  dplyr::mutate(across(.cols = everything(.), as.character)) %>% 
  dplyr::left_join(., get_questions_opts(data = preguntas, var = "V40a", var_lab = "Label"), by = c("V40a" = "Opcoes_numero"))

df_tipo_org <- raw_list %>% 
  purrr::pluck(., grep("Empresas_orgs_atributos", names(raw_list))) %>% 
  dplyr::select(ID, V39a) %>% 
  dplyr::mutate(across(.cols = everything(.), as.character)) %>% 
  dplyr::left_join(., get_questions_opts(data = preguntas, var = "V39a", var_lab = "Label"), by = c("V39a" = "Opcoes_numero"))

df_subtipo_org <- raw_list %>% 
  purrr::pluck(., grep("Empresas_orgs_atributos", names(raw_list))) %>% 
  dplyr::select(ID, V39b) %>% 
  dplyr::mutate(across(.cols = everything(.), as.character)) %>% 
  dplyr::left_join(., get_questions_opts(data = preguntas, var = "V39b", var_lab = "Label"), by = c("V39b" = "Opcoes_numero"))


perc_var_names <- percepcoes_df_cleaned %>% 
  dplyr::select(-ID, -ID_ego, -contains("V1_2020"), -starts_with("V20_"), -contains("Observações") ) %>% 
  names()

lapply(perc_var_names, function(i){
  
  percepcoes_df_cleaned[i] <<- percepcoes_df_cleaned %>% 
    dplyr::select(v_lab = !!sym(i)) %>%
    dplyr::mutate(across(.cols= everything(.), as.character)) %>% 
    dplyr::left_join(., get_questions_opts(data =preguntas, var = i, var_lab = i), by = c("v_lab" = "Opcoes_numero")) %>% 
    dplyr::select(-v_lab)
    
}) 


percepcoes_df_cleaned %>% 
  dplyr::mutate(ID = as.character(ID)) %>% 
  #left_join(., entrevistados %>% dplyr::select(ID_node, V1), by =c("ID" = "ID_node") ) %>% 
  left_join(., df_sec_eco %>% dplyr::select(ID, Economic_sector= Label), by = "ID") %>% 
  left_join(., df_tipo_org %>% dplyr::select(ID, Org_type= Label), by = "ID") %>% 
  left_join(., df_subtipo_org %>% dplyr::select(ID, Org_subtype= Label), by = "ID") %>% 
  dplyr::select(ID, ID_ego, V1_2020, Economic_sector, Org_type, Org_subtype, everything(.) ) %>%
  dplyr::mutate(Economic_sector = ifelse(is.na(Economic_sector), "No information", Economic_sector)) %>% 
  writexl::write_xlsx(., "D:/OneDrive - CGIAR/Documents/PPA 2021/percepciones con actividad economica 2020.csv")
