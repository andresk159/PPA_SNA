...
inv_2019 <- raw_list_2019[[grep("_colab", names(raw_list_2019))]]
...
d <- nt1 %>% 
  dplyr::select( R_Start_ID,  R_Destiny_ID, R4a, R4b, R_Start_ID_ego) %>%
...
nt_all <- bind_rows(d, nt2) %>% 
  dplyr::mutate(R4b = if_else(is.na(R4b), "9", R4b))%>% 
...
net_felipe_ego_grC <- net_graphs(edge_list = nt_all, 
                                 ...
                                 out_file = paste0(out_dirs[[name_rodada]], "/colabInst_grC_net.png"))
...
writexl::write_xlsx(to_save, path = paste0(out_dirs[[name_rodada]],"/colabInst_grC_net_results.xlsx"))


##colabInst no tiene direccion
...
d2 <- nt1 %>%
  ...
  #dplyr::filter(R4a != 2) %>%
  #dplyr::filter(R4b != 9) %>% 
  dplyr::select(R_Start_ID, R_Destiny_ID, starts_with("R4"))
...
net_felipe_ego <-net_graphs(edge_list = d2, 
                            ...
                            gr_colors = gr_colors[[name_rodada]], 
                            preguntas = preguntas[[name_rodada]], 
                            out_file = paste0(out_dirs[[name_rodada]], "/colabInst_standard_net_graph.png"))
...
net_graphs_orgtype(edge_list = d2, 
                   ...
                   type_org = type_org[[name_rodada]], 
                   preguntas = preguntas[[name_rodada]], 
                   out_file = paste0(out_dirs[[name_rodada]], "/colabInst_standard_net_graph_orgtype.png"))
...
writexl::write_xlsx(to_save, path = paste0(out_dirs[[name_rodada]],"/colabInst_standard_net_results.xlsx"))
...
inv_2020 <- raw_list_2020[[grep("_colab", names(raw_list_2020))]]
...
saveRDS(net,  paste0(out_dirs[["rodada_2019"]],"/colabInst_edge_list_JF.rds"))
...
inv_2021 <- raw_list_2021[[grep("_colab", names(raw_list_2021))]]
...
d <- nt1 %>%
  dplyr::select( R_Start_ID,  R_Destiny_ID, R4a,R4c, R4b, R_Start_ID_ego) %>%
  dplyr::mutate_all(as.character)
...
inv_2022 <- raw_list_2022[[grep("_colab", names(raw_list_2022))]]
